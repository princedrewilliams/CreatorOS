...
export function ChannelProfile() {
  return <GlassCard className="w-full p-8" delay={0.1} hoverEffect={true}>
        ...
        <div className="flex flex-col md:flex-row items-center gap-6 text-center md:text-left flex-1">
          <div className="relative group cursor-pointer">
            ...
            <div className="relative">
              <img src="https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=200&h=200&fit=crop" alt="Channel Avatar" className="relative h-24 w-24 rounded-full border-2 border-white/20 object-cover shadow-2xl z-10" />
              <motion.div initial={{
              scale: 0
            }} animate={{
              scale: 1
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-sm text-white/60 mb-3">
              <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 hover:bg-white/10 transition-colors border border-white/5">
                ...
                <span className="font-medium text-white">14.2M</span>{' '}
                subscribers
              </div>
              <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 hover:bg-white/10 transition-colors border border-white/5">
                <Eye className="w-3.5 h-3.5 text-blue-400" />
                <span className="font-medium text-white">7.8B</span> views
              </div>
              <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 hover:bg-white/10 transition-colors border border-white/5">
                <Calendar className="w-3.5 h-3.5 text-pink-400" />
                <span className="font-medium text-white">Daily</span> uploads
              </div>
            </div>
            ...
            delay: 0.4
              ...
              Position around a clear promise and niche proof. Videos
              consistently outperform peer channels of similar size.
            </motion.p>
          </div>
        </div>
        ...
        <div className="flex-shrink-0">
          ...
          rotate: 5
            ...
            <svg className="absolute inset-0 h-full w-full -rotate-90 transform" viewBox="0 0 100 100">
              ...
              <motion.circle className="text-pink-500 drop-shadow-[0_0_8px_rgba(236,72,153,0.8)]" strokeWidth="6" strokeLinecap="round" stroke="currentColor" fill="transparent" r="44" cx="50" cy="50" initial={{
              strokeDasharray: '276 276',
              strokeDashoffset: 276
            }} animate={{
              strokeDashoffset: 276 - 276 * 0.71
            }} transition={{
              duration: 2,
              ease: 'easeOut',
              delay: 0.5
      ...
      <div className="mt-8 pt-8 border-t border-white/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          <MetricBar label="Discoverability" value={86} color="bg-blue-500" delay={0.6} />
          <MetricBar label="Consistency" value={95} color="bg-purple-500" delay={0.7} />
          <MetricBar label="Engagement" value={22} color="bg-yellow-500" delay={0.8} />
          <MetricBar label="Winning Topics" value={98} color="bg-green-500" delay={0.9} />
        </div>
      </div>
    </GlassCard>;
}
function MetricBar({
  ...
  return <div className="flex flex-col gap-2 group cursor-pointer" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      ...
      <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden relative">
        <motion.div className={`h-full ${color} rounded-full shadow-[0_0_10px_rgba(255,255,255,0.3)] relative overflow-hidden`} initial={{
        width: 0
      }} animate={{
        width: `${value}%`
      }} transition={{
